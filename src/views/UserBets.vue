      ...
      <template slot-scope="{data}">
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="product-category">{{ tr.betAmount }} {{ tr.walletName }}</p>
            </vs-td>

            <vs-td>
              <vs-chip
                v-if="tr && tr.radar && tr.radar.match"
                :color="tr.radar.match.status.name.toLowerCase() == 'ended' ? 'success' : 'warning'"
                class="product-order-status"
              >{{ tr.radar.match.status.name }}</vs-chip>
               <vs-chip
                v-else-if="tr.time_status == 0 && parseInt(tr.eventStartDate)< (Math.round((new Date()).getTime() / 1000) + 600)"
                color="warning"
                class="product-order-status"
              >TIME PASSED</vs-chip>
              <vs-chip
                ...
                class="product-order-status"
              >NOT STARTED</vs-chip>
              <vs-chip v-else-if="tr.time_status == 1" color="danger" class="product-order-status">INPLAY</vs-chip>
              <vs-chip
            ...
            <vs-td v-if="false">
              <template v-if="tr.result">
                ...
                >
                  <template
                    v-if="parseInt(tr.result.time) < (Math.round((new Date()).getTime() / 1000) + 3600)"
            ...
            <vs-td>
              <vs-chip class="product-order-status">{{ tr.status2 ? tr.status2.toUpperCase() : '-' }}</vs-chip>
            </vs-td>
