...
<template>
  <vs-sidebar v-if="data" click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <div class="img-container w-100 mx-auto flex items-center justify-center">
        ...
        <v-status-select v-if="betStatus" as="name::id"  v-model="betStatus" :from="order_status_choices" :filter="false" :find="false"></v-status-select >
        <div class="w-100 pt-2">Type: {{ data.type }}</div>
        <div class="w-100 pt-2">Username:<a :href="'/user-edit/'+data.username" target="_blank">{{ data.username }}</a></div>



        <div v-if="data.type == 'WITHDRAW'" class="w-100 pt-2">TX ID:
          <vs-input class="w-full" v-model="txid" name="txid" />

        </div>
        <div v-if="data.txid" class="w-100 pt-2">TX ID:
          <a v-if="data.walletType == 'BTC'" target="_blank" :href="'http://blockchain.com/btc/tx/' + data.txid">{{ data.txid.substring(0,15) }}..</a>
          <a v-else-if="data.walletType == 'ACED'" target="_blank" :href="'https://explorer.acedcoin.com/tx/' + data.txid">{{ data.txid.substring(0,15) }}..</a>
        </div>




        <div class="w-100 pt-2">Address: {{ data.address }}</div>
...
export default {
  ...
  watch: {
    isSidebarActive(val) {
      if(!val) return;
      this.betStatus = this.order_status_choices.find(s=>s.id == this.data.status);
...
order_status_choices: [
  ...
  methods: {
    ...
    submitData(){
      if(!this.betStatus || this.data.status == this.betStatus.id)
        return;
      ...
      axios.post("/updateusertx", {userid:this.data.userid,oldstatus:this.data.status,status:this.betStatus.id,id:this.data.id,txid:this.txid})
        ...
        .then((response) => {
          ...
          if(response.res > 0){
            ...
            this.$vs.notify({title:'Warning',text:'Error!' + response.messages,color:'warning',icon:'check_box'})
          }
